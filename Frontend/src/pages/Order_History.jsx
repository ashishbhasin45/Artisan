...
export default function OrderHistoryPage() {
    ...
    const orderHistoryBackendURL= import.meta.env.VITE_REACT_APP_BACKEND_URL+ "api/v1/order/history";
    const addFeedbackURL= import.meta.env.VITE_REACT_APP_BACKEND_URL+ "api/v1/ratings/";
    ...
    const submitFeedback = async (productId, rating, feedback) => {
        try {
          ...
          await axios.post(`${addFeedbackURL}${productId}`, {
            rating: rating,
            comment: feedback
          }, {
    ...
    return (
      ...
      {orders.length > 0 && orders.map(order => (
        <div className="rounded-lg border border-gray-300 shadow-md mb-8 p-6" key={order.orderId}>
          <div className="flex justify-between items-center mb-4">
            <span className="text-lg font-semibold">Order Number: {order.orderId}</span>
            <span className="text-gray-500">Order Date: {new Date(order.orderDate).toLocaleDateString()}</span>
          </div>
                  ...
                  <tr key={item.product.id}>
                    <td className="px-4 py-2">
                      <div className="flex items-start space-x-4">
                        <img src={item.product.imageUrls[0]} alt={item.product.name} className="w-12 h-12 object-cover rounded-lg" />
                        <div>
                          <span className="block font-semibold">{item.product.name}</span>
                          <span className="text-gray-500">{item.product.categoryName}</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-2 text-center">
                      <button className="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded" onClick={() => openFeedbackForm(item.product.id)}>Add Feedback</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="mt-2">
            ...
            <span className="ml-2">{order.paymentDetail.provider} - {order.paymentDetail.card_number}</span>
          </div>
          <div className="mt-2">
            <span className="font-semibold">Shipping Address:</span>
            <span className="ml-2">{order.shippingBillingDetail.line1}, {order.shippingBillingDetail.city}, {order.shippingBillingDetail.state}, {order.shippingBillingDetail.country}, {order.shippingBillingDetail.postal_code}</span>
          </div>
        </div>
      ))}
      {orders.length == 0 &&
      ...
      {feedbackForm !== null && (
        <FeedbackForm productId={feedbackForm} onClose={closeFeedbackForm} onSubmit={submitFeedback} />
      )}
    </div>
...
const FeedbackForm = ({ productId, onClose, onSubmit }) => {
  ...
  const submitArtistFeedback = async () => {
    try {
      const token = localStorage.getItem("token");
      const productDetails = orders.find(order => order.items.some(item => item.product.id === productId));
      const artistId = productDetails.items.find(item => item.product.id === productId).product.artistId;
      await axios.post(`${addFeedbackURL}artists/${artistId}`, {
        rating: artistRating,
  ...
  const [orders, setOrders] = useState([]); 
    ...
    return (
        ...
        <div className="bg-white p-8 rounded-lg shadow-md" style={{ width: '80%' }}>
          <h2 className="text-xl font-semibold mb-4">Provide Feedback</h2>
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              <label className="block text-sm font-semibold mb-2">Product Rating:</label>
              ...
              {[1, 2, 3, 4, 5].map(starValue => (
                <input
                  ...
                  type="radio"
                  name="product-rating"
                  className={`mask mask-star-2 bg-orange-400 ${starValue === productRating ? 'checked' : ''}`}
            ...
            <div className="mb-4">
              <label className="block text-sm font-semibold mb-2">Product Feedback:</label>
              <textarea
            ...
            <div className="mb-4">
              <label className="block text-sm font-semibold mb-2">Artist Rating:</label>
              ...
              {[1, 2, 3, 4, 5].map(starValue => (
                <input
                  ...
                  type="radio"
                  name="artist-rating"
                  className={`mask mask-star-2 bg-orange-400 ${starValue === artistRating ? 'checked' : ''}`}
            ...
            <div className="mb-4">
              <label className="block text-sm font-semibold mb-2">Artist Feedback:</label>
              <textarea
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
                rows="4"
